...
class NumberDFS<E extends IEdge<V>, V extends IVertex> extends ParentAndPathDFS<E, V> {
     ...
     *               the according order.
     */
    NumberDFS(IGraph<E, V> graph, MetaInfoContainer container, NodeMap<V> adjMap) {
    ...
    protected void postVisit(V v, int dfsNumber, int complNumber) {
        ...
        lowpt1NumMap.put(v, numVMap.get(((NodeMap<V>) meta.getMetaInfo(MetaInfo.DFS_LOWPT1_VERTEX)).get(v)));
        lowpt2NumMap.put(v, numVMap.get(((NodeMap<V>) meta.getMetaInfo(MetaInfo.DFS_LOWPT2_VERTEX)).get(v)));
    }

    NodeMap<V> getHighptMap() {
        return highptMap;
    }
}