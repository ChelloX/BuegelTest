...
public class SplitCompDFS<E extends IEdge<V>, V extends IVertex> extends AbstractDFS<E, V> {
    ...
    private final EdgeMap<E, V> virtEdge;
    // edge: the virtual edge identifying the split component
    // to which each edge belongs
    private final EdgeMap<E, V> assignedVirtEdge;
    private final EdgeMap<E, V> edgeType;
    private final EdgeMap<E, V> isHidden;
    private final NodeMap<V> parent;
     ...
     */
    SplitCompDFS(IGraph<E, V> graph, MetaInfoContainer container,
     ...
     */
    void doShowDebugInformation(boolean b) {
     ...
     */
    private void updateTStack(V v, V w, boolean isTreeEdge) {
        ...
        if (isTreeEdge) {
            while ((!tStack.isEmpty()) && (tStack.peek() != EOS) &&
                    (tStack.peek().numA > getL1Num(w))
                    ) {
                lastRemoved = tStack.pop();
     ...
     */
    private void checkType1(E eBacktrack, V v, V w) {
        if (showDebugInformation) {
        ...
        if ((getL2Num(w) >= getNum(v))
            ...
            EdgeList<E, V> C = newComponent(new EdgeList<>());
            E virtualEdge;
            int numW = getNum(w);
            ...
            while ((!eStack.isEmpty()) && (((numW <= getNum(e.getV1())) && (getNum(e.getV1()) <= h))
                    || (((numW <= getNum(e.getV2())) && (getNum(e.getV2()) <= h))))) {
                // add edge to component
                e = eStack.pop();
                if (isHidden.getBool(e)) {
                    if (showDebugInformation) System.out.println(e + " is hidden!");
                }
                C = addToComponent(new EdgeList<>(e), C);
                if (!eStack.isEmpty()) {
                    e = eStack.peek();
                }
            }
            ...
            if (!eStack.isEmpty()) {
                ...
                if (isSameEdge(e, v, lowpt1W)) {
                    ...
                    e = eStack.pop();
                    EdgeList<E, V> el = new EdgeList<>(e);
                    el.add(virtualEdge);
                    C = newComponent(el);
                    virtualEdge = newVirtualEdge(C, v, lowpt1W);
                    // assign virtual edge
            ...
            if (lowpt1W != parent.get(v)) {
                ...
                EdgeList<E, V> el = new EdgeList<>(treeArcOfV);
                el.add(virtualEdge);
                C = newComponent(el);
     ...
     * @param w          the target vertex of eBacktrack
     */
    private void checkType2(E eBacktrack, V v, V w) {
        TSItem topTriple = null;
        if (!tStack.isEmpty()) {
            topTriple = tStack.peek();
        }
        ...
        if (!adjOfW.isEmpty()) {
            firstChildOfW = adjOfW.peek().getOtherVertex(w);
        }
        ...
        while ((v != dfsRoot) &&
            ...
            if ((topTriple.a == v)
                ...
                if ((edgeCountOfW == 2) && (firstChildOfW != null) && (getNum(firstChildOfW) > getNum(w))) {
                    // simple case
                    if (showDebugInformation) System.out.println("backtracking over edge " + eBacktrack +
     ...
     */
    private EdgeList<E, V> newComponent(EdgeList<E, V> compEdges) {
     ...
     */
    private EdgeList<E, V> addToComponent(EdgeList<E, V> compEdges, EdgeList<E, V> component) {
        removeEdges(compEdges);
     ...
     */
    private void removeEdges(EdgeList<E, V> edges) {
        for (E e : edges) {
            // remove edge from adjacency list
            // should be last element by imposed ordering
            EdgeList<E, V> adj = ((EdgeList<E, V>) ((NodeMap<V>) meta.getMetaInfo(MetaInfo.DFS_ORDERED_ADJ_LISTS)).get(e.getV1()));//DFS_ADJ_LISTS
            if (!adj.isEmpty()) {
     ...
     */
    private E newVirtualEdge(EdgeList component, V v, V w) {
        // create virtual edge and add edges to component
        E virtualEdge = (E) ((TCTreeSkeleton) g).addVirtualEdge(v, w);
        updateEdgeCount(v, 1);
        updateEdgeCount(w, 1);
        ...
        if (showDebugInformation)
            System.out.println(((NodeMap<V>) meta.getMetaInfo(MetaInfo.DFS_ORDERED_ADJ_LISTS)).get(v));//DFS_ADJ_LISTS
        ((EdgeList<E, V>) ((NodeMap<V>) meta.getMetaInfo(MetaInfo.DFS_ORDERED_ADJ_LISTS)).get(v)).add(virtualEdge);//DFS_ADJ_LISTS
        if (showDebugInformation) {
            System.out.println(((NodeMap<V>) meta.getMetaInfo(MetaInfo.DFS_ORDERED_ADJ_LISTS)).get(v));//DFS_ADJ_LISTS
     ...
     */
    private void makeTreeEdge(E e, V v, V w) {
    ...
    private boolean isSameEdge(E e, V v, V w) {
        return ((e.getV1() == v) && (e.getV2() == w)) || ((e.getV1() == w) && (e.getV2() == v));
    }
